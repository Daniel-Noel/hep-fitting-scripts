...
def match_model(model, fit_results_main):
    ...
    model_postfit = cabinetry.model_utils.prediction(
        model, fit_results=fit_results_matched
    )
...
def plot_datamc(model_postfit, data, region, var_name, plot_info):
    ...
    for txt in fig.texts:
        txt.set_visible(False)  # remove text on there currently

    out_dir = os.path.join("output", "plots", region)
    os.makedirs(out_dir, exist_ok=True)
    plt.savefig(
        os.path.join(out_dir, "{}_postfit.png".format(var_name)), transparent=False
    )
    plt.savefig(
        os.path.join(out_dir, "{}_postfit.pdf".format(var_name)), transparent=False
    )
